...
Taken from Stack Overflow https://stackoverflow.com/questions/25709398/set-location-of-special-folders-with-powershell  
...
'@
    ...
    if (Test-Path $Path -PathType Container) {
        ...
        $current = [Environment]::GetFolderPath($EnviroKnownFolder)
        if ($current -eq $Path) {
